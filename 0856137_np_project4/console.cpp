...
// #include "lib/sock_server/socks4_client.hpp"
#include <bits/stdc++.h>
#include <unistd.h>
#include <utility>
...
int main()
{
    ...
    // cout << sh << sh.size() << endl << flush;
    bool isUsingSocks4 = false;
    if (sh.size() > 0 && sp.size() > 0)
        isUsingSocks4 = true;
    for (int i = 0; i < Max_session_num; ++i) {
        // cout << i << endl;
        if (sessions[i].used) {
            auto ptr = make_shared<Session>(global_io_service, sessions[i], sh, sp, isUsingSocks4);
...
void parse_query_string(SessionInfo* session, string query)
{
    ...
    for (int i = 0; i < Max_session_num && !query.empty(); ++i) {
        ...
        if (session[i].host.empty() || session[i].port == 0 || session[i].file.empty())
            ...
            session[i].used = true;
        // cout << session[i].host << " " << session[i].port << " " << session[i].file << "\r" << flush;
    }
    // cout << query << endl << flush;
    pos = query.find_first_of("&");
    sh = query.substr(3, pos - 3);
    query.erase(0, pos + 1);
    pos = query.find_first_of("&");
    sp = query.substr(3, pos - 3);
    // cout << "sh: " << sh << " sp: " << sp << "\r" << flush;
}
